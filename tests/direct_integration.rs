...
// The committed `redis::replay::hget` and `redis::replay::hmget`
...
use redis::replay::{ArrayElement, BulkResponse, hget, hmget};
...
/// on the public `redis::replay::hget` / `redis::replay::hmget`
/// API).
...
async fn call_with_retry<F, T>(mut f: F) -> Result<T, redis::replay::RedisError>
where
    F: FnMut() -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Result<T, redis::replay::RedisError>> + Send>,
    >,
{
